...
func rstEnrollUser(w http.ResponseWriter, r *http.Request) {
	...
	// 新版eth
	nonce, err := client.PendingNonceAt(context.Background(), curUserAddr)
...
func rstPopulateContract(w http.ResponseWriter, r *http.Request) {
	...
	// 新版本eth必须有以下规范
	nonce, err := client.PendingNonceAt(context.Background(), common.HexToAddress("0xD24196422b6163D7DdB49F740DDD18945496CF33"))
	auth.Nonce = big.NewInt(int64(nonce))
	auth.Value = big.NewInt(10) // in wei
	auth.GasLimit = uint64(2000000)
	gasPrice, err := client.SuggestGasPrice(context.Background())
...
func rstDownloadCaCert(w http.ResponseWriter, r *http.Request) {
	...
	/*isCertOK*/
	_ /*revokDate*/, _ /*parentAddr*/, _ /*retCaHash*/, _, certData, err :=
...
func GetEventReturn(dataHash [32]byte, contrAddr common.Address) (result uint64, err error) {
	...
	client, err := ethclient.Dial("ws://127.0.0.1:2001")
	if err != nil {
