...
func EnrollUser(w http.ResponseWriter, r *http.Request) {
	...
	err := r.ParseMultipartForm(32 << 20)
	if err != nil {
		fmt.Printf("EnrollUser1: No change data -- %v\n", err.Error())
		http.Error(w, GeneralError{fmt.Sprintf(
			"EnrollUser2: No change data -- %v", err.Error())}.Error(),
			http.StatusInternalServerError)
		return
	}
	...
	ethClient, err := ethclient.Dial(gConfig.NodeURL)
	if err != nil {
...
func PkiForm(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	client, err := ethclient.Dial(gConfig.NodeURL)
	if err != nil {
	...
	for i := int64(0); i < numRD.Int64(); i++ {
		...
		if (regDatum.ContrAddr != common.Address{}) {
			...
			strContrAddr = common.Bytes2Hex(regDatum.ContrAddr.Bytes())
			isCA = true
		}
	...
	formParam.ParentAddr = common.Bytes2Hex(parentAddr.Bytes())
	formParam.SuperParentAddr = common.Bytes2Hex(superParentAddr.Bytes())
	if (superParentAddr == common.Address{}) {
		formParam.IsRoot = true
	}
	formParam.SuperUserAddr = common.Bytes2Hex(superUserAddr.Bytes())
	terr := tmpl.Execute(w, formParam)
	if terr != nil {
		http.Error(w, terr.Error(), http.StatusInternalServerError)
	}
...
func PopulateContract(w http.ResponseWriter, r *http.Request) {
	...
	detailStr += "Parent Addr = " + common.Bytes2Hex(parentAddr.Bytes()) + "\n"
	detailStr += "New User = " + common.Bytes2Hex(newUserAddr.Bytes()) + "\n"
	detailStr += "Current User = " + common.Bytes2Hex(curUserAddr.Bytes()) + "\n"
	if gConfig.WebMode != 0 {
		...
		if (curUserAddr == common.Address{}) && (newUserAddr == common.Address{}) { /* Root populating */
			...
			formParam.Params = append(formParam.Params,
				CParam{"ParentAddr", common.Bytes2Hex(parentAddr.Bytes())})
			formParam.Params = append(formParam.Params,
				CParam{"CurrentUserAddr", common.Bytes2Hex(curUserAddr.Bytes())})
			formParam.Params = append(formParam.Params,
				CParam{"NewUserAddr", common.Bytes2Hex(newUserAddr.Bytes())})
...
func CreateContract(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	auth, err := bind.NewTransactorWithChainID(strings.NewReader(string(key)), gConfig.Pswd, big.NewInt(1337))
	if err != nil {
		log.Fatalf("Failed to create authorized transactor: %v", err)
	}

	// 新版本eth发送交易
	nonce, err := client.PendingNonceAt(context.Background(), execUserAddr)
	auth.Nonce = big.NewInt(int64(nonce))
	auth.Value = big.NewInt(10) // in wei
	...
	*/
	...
	if gConfig.WebMode != 0 {
		...
		formParam.Params = append(formParam.Params,
			CParam{"ContrAddr", common.Bytes2Hex(contrAddr.Bytes())})
		formParam.Params = append(formParam.Params,
